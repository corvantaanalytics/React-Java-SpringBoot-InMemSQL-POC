...
public class ServiceApartmentsController {
	...
	@DeleteMapping(path = HostellerLinks.DELETE_SERVICEAPARTMENTS)
	public ResponseEntity<?> deleteServieApartment(@PathVariable("id") int id) {
		log.info("ServiceApartmentsController: deleteServiceApartment :: " + id);
		ServiceApartments resource = serviceApartmentsService.deleteServiceApartment(id);
		return ResponseEntity.ok(resource);
	}
